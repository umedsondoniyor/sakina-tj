...
const AdminProductVariants = () => {
  const [variants, setVariants] = useState<(ProductVariant & { product_name?: string })[]>([]);
  ...
  const filteredVariants = filterType === 'all' 
    ? variants 
    : variants.filter(v => v.size_type === filterType);
              ...
              {filteredVariants.map((variant) => (
                <tr key={variant.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    ...
                    {variant.size_type === 'mattress' && variant.width_cm && variant.length_cm && 
                      `${variant.width_cm}×${variant.length_cm}cm`}
                    {variant.size_type === 'bed' && variant.width_cm && variant.length_cm && 
                      `${variant.width_cm}×${variant.length_cm}cm`}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <div>
                      ...
                      {variant.old_price && (
                        <div className="text-xs text-gray-500 line-through">
                          {variant.old_price.toLocaleString()} c.
                        </div>
                      )}
                    </div>
                  </td>
