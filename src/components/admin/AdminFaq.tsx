...
const AdminFaq = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<FaqItem | undefined>();
  ...
  const toggleActive = async (row: FaqItem) => {
    try {
  ...
  const moveItem = async (id: string, direction: 'up' | 'down') => {
    ...
    try {
      ...
      await Promise.all([
        supabase.from('faq_items').update({ order_index: b.order_index }).eq('id', a.id),
        supabase.from('faq_items').update({ order_index: a.order_index }).eq('id', b.id),
      ]);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Частые вопросы (FAQ)</h1>
        <button
          type="button"
          onClick={handleAdd}
      ...
      {items.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-semibold text-gray-900">Пока нет вопросов</h3>
          <p className="mt-1 text-sm text-gray-500">Добавьте первый вопрос для страницы FAQ.</p>
        </div>
      ) : (
          ...
          {items.map((row, index) => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-600 mt-2 line-clamp-2">{row.answer}</p>
